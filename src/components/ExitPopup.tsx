...
export default function ExitPopup() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4" onClick={dismiss}>
      ...
      <div
        className="relative bg-dark-900 border border-white/10 rounded-2xl p-8 max-w-md w-full animate-scale-in"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={dismiss}
          className="absolute top-4 right-4 p-1.5 rounded-lg text-dark-300 hover:text-white hover:bg-white/10 transition-colors"
        >
          <X className="w-5 h-5" />
        </button>

        <div className="text-center">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-brand-500/20 to-accent-500/20 border border-brand-500/30 flex items-center justify-center mx-auto mb-6">
            <Gift className="w-8 h-8 text-brand-400" />
          </div>
          <h3 className="text-2xl font-bold text-white mb-3">Wait! Get a Free Store Audit</h3>
          ...
          <button onClick={dismiss} className="mt-3 text-dark-400 text-sm hover:text-dark-300 transition-colors">
            No thanks, I'll pass
          </button>
        </div>
      </div>
